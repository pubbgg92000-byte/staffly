...
const EnvSchema = z
  ...
  .superRefine((env, ctx) => {
    ...
    // Dev-flavored defaults are fine locally but silently catastrophic in
    // production: cookies scoped to `localhost` never reach the browser
    // (auth "works" server-side, every login loops), emailed reset/invite
    // links point at localhost, and mail is sent from a non-routable
    // `.local` sender. Same philosophy as the mailer factory: an API that
    // refuses to boot is strictly easier to notice than one that limps.
    ...
    if (env.COOKIE_DOMAIN === "localhost") {
      ctx.addIssue({
        ...
        path: ["COOKIE_DOMAIN"],
        message:
          "COOKIE_DOMAIN is 'localhost' in production — set it to the public site domain (cookies would never be sent by browsers)",
      });
    }
    if (/^https?:\/\/(localhost|127\.0\.0\.1)([:/]|$)/.test(env.APP_BASE_URL)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["APP_BASE_URL"],
        message:
          "APP_BASE_URL points at localhost in production — emailed reset/invite links would be unusable; set the public portal URL",
      });
    }
    if (/@staffly\.local>?$/.test(env.EMAIL_FROM)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["EMAIL_FROM"],
        message:
